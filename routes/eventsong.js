...
router.get('/:id/cover', passport.ensureAuthenticated, passport.ensureNotAnonymous, function (req, res, next) {
    // get event of current user - user is DJ
    db.Event.findOne( { owner_id: req.user.id, end:null}, function(err, event) {
        if (err) {
            console.log(err);
        ...
        // get song of file_id parameter
        if(event) {
            db.Song.findOne( { cover: req.param('id'), active: true}, function(err, song) {
                if (err) {
                    console.log(err);
                    res.status(500).send('Internal server error');
                    return;
